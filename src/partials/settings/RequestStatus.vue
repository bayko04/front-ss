                        ...
                        <div v-for="(item, index) in referencesStore.chatStatuses" :key="index" class="table-row">
                          ...
                          <div class="table-cell py-1 px-2" v-if="!item.editing && item.status_type !== 'standard'"
                            ...
                            <div class="table-cell py-1 px-2">
                                <button v-if="!item.editing && item.status_type !== 'standard'" @click="editItem(item)"
                                ...
                                <button v-if="item.editing && item.status_type !== 'standard'" @click="saveChanges(item)" class="btn dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600">
                            ...
                            <div class="table-cell py-1 px-2">
                                <button v-if="!item.editing && item.status_type !== 'standard'" @click="deleteChatStatus(item)"
                                        class="btn dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600">
                                    <svg class="w-4 h-4 fill-current text-rose-500 shrink-0" viewBox="0 0 16 16">
                                        <path d="M5 7h2v6H5V7zm4 0h2v6H9V7zm3-6v2h4v2h-1v10c0 .6-.4 1-1 1H2c-.6 0-1-.4-1-1V5H0V3h4V1c0-.6.4-1 1-1h6c.6 0 1 .4 1 1zM6 2v1h4V2H6zm7 3H3v9h10V5z"/>
                                    </svg>
                                </button>
                                <button v-if="item.editing && item.status_type !== 'standard'" @click="cancelEdit(item)"
                                        class="btn dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600">
