      ...
      <div class="grid gap-5 md:grid-cols-1">
        <div>
          ...
          <div class="table w-full">
            <div
              v-for="(item, index) in referencesStore.chatStatuses"
              :key="index"
              class="table-row"
            >
              <div v-if="!item.editing" class="table-cell py-1">
                {{ item.name }}
              </div>
              ...
              >
                ...
                <div class="relative">
                  <input
                    id="link-checkbox"
                    type="checkbox"
                    :checked="item.close_status === true"
                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    @click="finalStatus(item, $event)"
                  />
                  <!-- Описание при наведении -->
                  <div
                    class="absolute top-0 left-0 ml-2 mt-1 bg-gray-200 p-1 text-xs rounded shadow"
                    v-if="item.isHovering"
                    style="z-index: 999; left: 15px"
                  >
                    Статус для завершения чата
                  </div>
                </div>
              </div>
              <div class="table-cell py-1 px-2" v-if="!item.is_default">
                <button
                  v-if="!item.editing"
                  @click="editItem(item)"
                ...
                >
                  <svg
                    class="w-4 h-4 fill-current text-slate-500 dark:text-slate-400 shrink-0"
                    viewBox="0 0 16 16"
                  >
                    <path
              ...
              <div class="table-cell py-1 px-2" v-if="!item.is_default">
                ...
                >
                  <svg
                    class="w-4 h-4 fill-current text-red-600 shrink-0"
                    viewBox="0 0 24 24"
                  >
                    <path
                      stroke="#FF0000"
