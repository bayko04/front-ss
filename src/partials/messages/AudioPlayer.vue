            ...
            <div class="md:flex justify-between items-center text-center md:text-left">
              ...
              <div class="max-w-3xl">
                ...
                <div class="relative px-4 py-2 min-w-[400px]" aria-label="Audio Player" role="region">
                  ...
                  <div class="sm:flex items-center sm:space-x-3">
                    ...
                    <div class="flex items-center space-x-3 mb-3 sm:mb-0">
                        ...
                        <svg :class="{ 'hidden': playing }" xmlns="http://www.w3.org/2000/svg" width="40" height="40" aria-hidden="true">
                          <defs>
                            <linearGradient id="play-a" x1="87.87%" x2="12.333%" y1="20.647%" y2="76.5%">
                              <stop offset="0%" stop-color="#475569" stop-opacity=".64" />
                      ...
                      <button aria-label="Playback speed" @click="toggleSpeed">
                        <div class="flex items-center justify-center h-5 w-6 bg-white bg-opacity-30 rounded text-center text-[10px] text-[#475569] font-bold">{{speedOutput[speedIndex]}}</div>
                      </button>
                    ...
                    <div class="grow flex items-center space-x-3">
                      ...
                      <!-- Seek bar -->
                      <div class="grow bg-[#475569] bg-opacity-50 overflow-hidden"
                        ref="slider"
                        tabIndex="0"
                        role="slider"
                        ...
                        @keydown="keyControl"
                      >
                        <div class="h-2 bg-gradient-to-bl from-[#475569] pointer-events-none" :style="`width: ${progress.value}%`"></div>
                      </div>

                      <!-- Time -->
                      <div class="flex items-center text-xs font-medium text-[#475569] space-x-1.5 tabular-nums">
                        <span>{{outputCurrentTime.value}}</span>
                        <span class="w-px h-4 bg-[#475569] bg-opacity-30" aria-hidden="true"></span>
                  ...
                  <audio id="audiofile" :src="audioPath" ref="audio" @loadedmetadata="loadedMetadata" @timeupdate="timeUpdate" @ended="ended"></audio>
...
export default {
  ...
  setup(props, context) {
    ...
    const loadedMetadata = () => {
      duration.value = audio.value.duration || 0; // Используйте значение duration, а не Math.floor
      outputDuration.value = formatTime(duration.value);
    }

    const formatTime = (seconds) => {
      if (isNaN(seconds) || !isFinite(seconds)) {
        return '0:00'; // Возвращаем значение по умолчанию, если seconds не является числом
      }
