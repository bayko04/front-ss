<template>
  <ul class="mb-6">
    <li class="-mx-2" v-for="chat in displayedChats" :key="chat.id">
      <button
        v-if="
          (chatSortStatus === 2 &&
            chat.latest_customer_request.user_id === null) ||
          chatSortStatus === 3 ||
          (chatSortStatus === 1 &&
            chat.latest_customer_request.user_id === authStore.userData.user.id)
        "
        ...
        :class="{
          'bg-indigo-500/30':
            ...
            chat.account_id === activeChat?.account_id,
        }"
      >
        <div class="flex items-center truncate">
          ...
          <div class="truncate flex flex-col items-start">
            ...
            <span
              ...
              >{{ lastMessage(chat.messages) }}</span
            >
            <span
              v-else-if="chat.latest_message"
              class="text-xs text-gray-500 truncate"
              >{{ chat.latest_message.text }}</span
            >
          </div>
        </div>
        <div v-if="chat.unread_messages_count" class="flex items-center ml-2">
          <div
