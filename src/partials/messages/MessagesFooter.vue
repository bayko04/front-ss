    ...
    <div v-show="activeAccount && activeChat && (!activeAccount.ai_active || activeChat.latest_customer_request.user_handle)">
      ...
      <div v-show="activeChat" class="flex items-center justify-between bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-4 sm:px-6 md:px-5" :style="{ height: blockHeight }">
        <div class="grow flex items-end mt-1">
          ...
          <button v-if="recording" @click="handleStopRecording" class="shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 mr-3 mb-1 dark:hover:text-slate-400 cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-player-stop w-6 h-6" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="none" stroke-linecap="round" stroke-linejoin="round">
              <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
          ...
          <button v-if="!recording && activeChat?.message.text" @click="send()"  type="submit" class="mb-1 btn bg-indigo-500 hover:bg-indigo-600 text-white whitespace-nowrap h-8">
