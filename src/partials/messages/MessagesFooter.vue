<template>
  <div class="sticky bottom-0">
    <div v-show="activeAccount && activeChat && (!activeAccount.ai_active || activeChat.latest_customer_request.user_handle)">
      ...
      <!-- wrapper -->
       <div class="">
        ...
        <div v-show="activeChat" class="flex w-[85%] relative  rounded-[10px] items-center justify-between bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-[10px] sm:px-6" :style="{ height: blockHeight }">
            <div class="absolute top-[-60px] left-0 w-full">
              ...
              <div class="grow flex items-center mt-1 py-[5px]">
                ...
                <div class="grow mr-3 hidden" v-show="recording">
                  <div id="audio" class="w-full bg-slate-100 dark:bg-slate-800 border-transparent dark:border-transparent"></div>
                </div>
                <button v-if="!recording" @click="openEmoji" class="shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 mr-3 mb-1 dark:hover:text-slate-400 cursor-pointer">
                ...
                <button v-if="recording" @click="handleCancelRecording" class="shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 mr-3 mb-1 dark:hover:text-slate-400 cursor-pointer">
                  <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-trash w-6 h-6" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                    <path d="M4 7l16 0" />
                    <path d="M10 11l0 6" />
                    <path d="M14 11l0 6" />
            ...
            <!-- voice -->
          <div class="h-[60px] flex-[0_0_60px] bg-[white] rounded-[50%] text-center hover:bg-[#6366F1] cursor-pointer transition ease duration-300 group">
            <button v-if="recording" @click="handleStopRecording" class="h-[60px] w-full shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400 cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-player-stop w-full h-6 group-hover:stroke-[white]" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                <path d="M5 5m0 2a2 2 0 0 1 2 -2h10a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-10a2 2 0 0 1 -2 -2z" />
              </svg>
            </button>
            <button v-else-if="!recording && !activeChat?.message.text && activeAccount?.messenger_id !== 3" @click="handleStartRecording" class="h-[60px]  w-full shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-500 dark:hover:text-slate-400 cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-microphone w-full h-6 group-hover:stroke-[white]" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
            ...
            <button v-if="!recording && activeChat?.message.text" @click="send()"  type="submit" class="h-[60px] w-full btn text-white whitespace-nowrap h-8">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-send-2 w-full group-hover:stroke-[white]" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="rgb(99 102 241)" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    ...
    <div class="flex items-center justify-between bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-4 sm:px-6 md:px-5" v-show="activeAccount && activeChat && activeAccount.ai_active && !activeChat.latest_customer_request.user_handle">
      <ul class="mb-4 mt-4 text-center">
        <p class="text-lg font-bold text-red-600 dark:text-red-400">
          Сейчас отвечает нейросеть. Чтобы начать отвечать, переключите на менеджера.
        </p>
        <div class="flex justify-center mt-4">
          <div @click="handleUser()" class="flex items-center justify-center cursor-pointer bg-blue-600 text-white py-1.5 px-2 rounded-lg hover:bg-blue-700 transition duration-300 w-48">
            <img :src="managerIcon" alt="Image description" class="w-4 h-4 filter invert">
...
onMounted(() => {
  ...
  record.value = wavesurfer.value.registerPlugin(RecordPlugin.create({ mimeType: "audio/webm;codecs=opus" }))
...
const blobToFile = function(blob) {
  return new File([blob], 'audio.' + (blob.type.split(';')[0].split('/')[1] || 'webm'), {
    type: blob.type,
  })
}
