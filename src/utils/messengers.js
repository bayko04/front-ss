...
export function useMessangers() {
  ...
  const startSocketListeners = async function () {
    ...
    if (activeAccount.value === undefined) {
      ...
      accountId
        ? setActiveAccount(getAccountById(accountId))
        : setActiveAccount();
    }
    ...
    if (activeCommentsChat.value === undefined) {
      ...
      chatId
        ? setActiveCommentsChat(getCommentsChatById(chatId))
        : setActiveCommentsChat(undefined);
    }
  ...
  function getAccountById(id) {
    const foundAccount = accounts.value.find(
  ...
  function getCommentsAccountById(id) {
    const foundAccount = accounts.value.find(
      (account) => account.id === Number(id) && account.messenger.id === 3
    );
    return foundAccount || undefined;
  }

  function getChatById(id) {
    const foundChat = activeAccount.value?.chats?.find(
  ...
  function getCommentsChatById(id) {
    const foundChat = activeCommentsAccount.value?.contents.find(
      (chat) => chat.id === Number(id)
    );
    return foundChat || undefined;
  }
  ...
  const setAccountChats = async function (account) {
    const accountChats = (
      await fetchWrapper.get(`/chat/accounts/${account.id}`)
    ).data;
  ...
  const setActiveCommentsAccount = async function (account = undefined) {
    if (!account) {
      activeCommentsAccount.value = accounts.value.find(
        (account) => account.messenger_id === 3
      );
    } else {
  ...
  const setActiveChat = async function (chat) {
    ...
    if (activeChat.value.latest_customer_request) {
      taskStore.getTasksForCustomerRequest(
  ...
  const setActiveCommentsChat = async function (chat) {
    ...
    const newRoute = {
      path: "/comments",
      query: {
        commentsAccountId: activeCommentsAccount.value?.id,
        commentsChatId: activeCommentsChat.value.id,
      },
    };
  ...
  const getComments = async function () {
    ...
    activeCommentsChat.value.comments = {
      ...
      ...(
        await fetchWrapper.get(
          `/${activeCommentsAccount.value?.messenger.name}/chats/${activeCommentsChat.value.id}/comments`
        )
      ).data,
    };
  };
  ...
  const addCommentToCommentsChat = function (account, content) {
    echo.value
      .private(`${account.messenger.name}.${account.id}.content`)
      .listen(`.${content.id}.NewComment`, function (socketComment) {
        if (
          content.comments &&
          content.id === activeCommentsChat.value.id &&
          socketComment.comment.parent_id != "" &&
          content.comments[socketComment.comment.parent_id]
        ) {
          if (!content.comments[socketComment.comment.parent_id]["replies"]) {
            content.comments[socketComment.comment.parent_id]["replies"] = [];
          }
          content.comments[socketComment.comment.parent_id]["replies"].push(
            socketComment.comment
          );
        } else if (content.comments) {
          content.comments[socketComment.comment.id_from_comment] =
            socketComment.comment;
        }
  ...
  const updateMessageInChat = function (account, chat) {
    echo.value
      .private(`${account.messenger.name}.${account.id}.chat`)
      .listen(`.${chat.id}.UpdateMessage`, function (socketMessage) {
        if (chat.messages) {
          if (
            chat.messages[socketMessage.message.id].message_status_id === 3 &&
  ...
  const scrollToBottom = function () {
    setTimeout(() => {
      ...
      if (messagesBottomElement) {
        messagesBottomElement.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }
  ...
  const newChatFromSocket = function (account) {
    echo.value
      .private(`${account.messenger_name}.${account.id}.chat`)
      .listen(".NewChat", function (socketChat) {
        ...
        fetchWrapper
          .get(
            `/${account.messenger.name}/chats/${chat["id"]}/unread-messages/count`
          )
  ...
  const newCommentsChatFromSocket = function (account) {
    echo.value
      .private(`${account.messenger.name}.${account.id}.content`)
      .listen(".NewContent", function (socketContent) {
  ...
  const updateChatFromSocket = function (chat, account) {
    echo.value
      .private(`${account.messenger.name}.${account.id}.chat`)
      .listen(`.${chat.id}.UpdateChat`, function (socketChat) {
  ...
  const removeChat = (account, chat) => {
    ...
    if (chatIndex !== -1) {
      ...
      echo.value.leave(
        `${account.messenger.name}.${account.id}.chat.${chat.id}`
      );
    }
  };
  ...
  const updateCommentsChatFromSocket = function (content, account) {
    echo.value
      .private(`${account.messenger.name}.${account.id}.content`)
      .listen(`.${content.id}.UpdateContent`, function (socketContent) {
  ...
  const saveMessage = async function () {
    ...
    await fetchWrapper.post(
      `/${activeAccount.value.messenger.name}/chats/${chatId}/new-message`,
  ...
  const replyComment = async function (text, repliedCommentId, contentId) {
    await fetchWrapper.post(
      `/${activeCommentsAccount.value.messenger.name}/new-comment`,
      { text: text, repliedComment: repliedCommentId, contentId: contentId }
    );
  };
  ...
  const replyToDirect = async function (form) {
    const result = (
      await fetchWrapper.post(
        `/${activeCommentsAccount.value.messenger.name}/reply-to-direct`,
        form
      )
    ).data;
  ...
  const searchChat = async function (search) {
    const chats = (
      await fetchWrapper.post(
        `/${activeAccount.value.messenger.name}/chats/search`,
        { search: search }
      )
    ).data;
  ...
  const markAsRead = async function (message) {
    await fetchWrapper.post(
      `/${activeAccount.value.messenger.name}/messages/${message.id}/mark-as-read`
    );
  };

  const openChat = async function (chatId) {
    await fetchWrapper.post(
      `/${activeAccount.value.messenger.name}/chats/${chatId}/open-chat`
    );
  };
  ...
  const sendFiles = async function () {
    ...
    const chatId = activeChat.value?.id;
    await fetchWrapper.post(
      `/${activeAccount.value.messenger.name}/chats/${chatId}/new-files-message`,
      formData
    );
    activeChat.value.message = JSON.parse(JSON.stringify(emptyMessage));
    setTimeout(() => {
